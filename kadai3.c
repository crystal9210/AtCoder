// 親プロセスから二つの子プロセスを生成し、１つは正規の子プロセス、暗号鍵を用いてプロセス間で暗号通信を簡易的に行う、また、親プロセスは復号鍵をもっていて、暗号化された内容を復号可能、また、もう1つ子プロセスを生成するが、これは正規の子プロセスが暗号化したコードを復号しようと、プロセス間通信を盗聴し、別の復号ロジックを試すが失敗するというのを再現しているプログラム。再帰的にプロセスが乱数で特定の数値が出るまで生成され続けるプログラムに使用かと考えたがマルウェアとみなされかねないので棄却。環境構築および実行は、たぶんできると思うので省略。
// 追記：mac環境でのコンパイルコマンド：gcc -o kadai3 kadai3.c -I/opt/homebrew/opt/openssl@3/include -L/opt/homebrew/opt/openssl@3/lib -lssl -lcrypto
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
